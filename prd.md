...
### Garmin Glance + Full-App UI (FEAT-06) — Color-Coded Watch Display with Scrollable Detail
...
- **Full-app detail view:** `CommuteBuddyView` displays a vertically stacked layout: color-coded action header (`FONT_LARGE`), summary (`FONT_SMALL`, wrapped), "Routes: N,W" line (omitted for NORMAL with empty routes), reroute hint (`FONT_TINY`, REROUTE only), freshness ("<1 min ago" / "N min ago" / "N hr ago" / "Stale") in light gray
- **Dynamic section heights:** each section's Y position is computed from the actual rendered height of the preceding section using a manual word-wrapping simulation (`dc.getTextWidthInPixels()` + `dc.getFontHeight()`) — no hardcoded Y offsets, no blank gaps
- **Swipe-to-scroll:** `CommuteBuddyDelegate` extends `WatchUi.BehaviorDelegate`, captures `SWIPE_UP`/`SWIPE_DOWN` events, and calls `view.scrollBy()` which adjusts a `_scrollOffset` and calls `WatchUi.requestUpdate()` — content that doesn't fit the 390×390 screen is accessible by swiping
...
│   └── source/
│       ├── CommuteBuddyApp.mc                      # AppBase: validates new BLE schema (action/summary/affected_routes/reroute_hint/timestamp), stores cs_* keys, clears stale cs_reroute_hint, requestUpdate; getInitialView() returns [view, delegate]
...
│       ├── CommuteBuddyView.mc                     # Full-app detail view: color-coded header, wrapped summary, routes, reroute hint (REROUTE only), freshness; dynamic Y via calcWrappedHeight(); _scrollOffset for swipe-to-scroll
│       └── CommuteBuddyDelegate.mc                 # BehaviorDelegate: captures SWIPE_UP/SWIPE_DOWN, calls view.scrollBy(±90) to drive scroll offset
├── shared/
...
### Bugs
- [ ] BUG-01: The garmin app full screen UI is not very good. A) The text from the summary is often truncated even though it's wrapped somewhat. B) The scrolling behavior is very choppy because it's not using the native OS scrolling handling, instead tracking swipes and redrawing manually, which is a terrible UX. This needs to be rebuilt properly using best practices, and requires some research, perhaps into how other apps do it.