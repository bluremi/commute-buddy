...
- [ ] FEAT-14: Add automatic commute direction handling so that the commute direction is passed to Gemini based on which polling window is currently active. Outside of an active polling window, use the last polled direction. Also improve the UX on the home screen of the android app so that the TO WORK and TO HOME buttons only control which direction is sent when you press FETCH LIVE. Improve the UX so that it is clear and intuitive how those settings work and that they relate to the FETCH LIVE button.
