...
- **Full-app detail view (BUG-01 + FEAT-13):** Native page navigation via `WatchUi.ViewLoop` + `ViewLoopFactory` + `ViewLoopDelegate`. `DetailPageFactory` builds a page model from storage, chunks long summaries via `DetailPagination.chunkSummary()`, and provides `DetailPageView` instances. Page 1 shows a structured header in this vertical order: action title (large, action-tier color) → colored route badges → freshness timestamp (FONT_XTINY, gray) → reroute hint if present (action-tier color). Summary text follows in white (FONT_XTINY). When the header fills the screen (e.g., a multi-line REROUTE hint), page 1 is header-only and all summary text starts on page 2. Subsequent pages show summary chunks at full-page height. The reroute hint height is dynamically measured using an iterative `Graphics.fitTextToArea()` loop (`fitted.length() >= original.length()` detects full fit vs truncation, since `fitTextToArea` injects `\n` line breaks). No manual scroll offset; swipes and buttons use framework delegates for smooth native transitions.
...
### Automatic Commute Direction (FEAT-14)
- **Auto-direction in background polling:** `PollingForegroundService.resolvePollingDirection()` derives direction from the active window index — window 0 → TO_WORK, window 1 → TO_HOME. The manual UI toggle is never consulted during background polls. Handles 0, 1, or 2 configured windows correctly.
- **Last-polled direction persisted:** when a background poll runs inside a window, the resolved direction is saved to `KEY_LAST_POLLED_DIRECTION` in SharedPreferences. Outside any window (e.g., service restart, off-hours poll), this persisted value is the fallback; fresh-install default is TO_WORK.
- **Manual fetch direction stays user-controlled:** the TO WORK / TO HOME toggle and Fetch Live button are grouped with a "Manual fetch direction:" label, making it clear the toggle only affects on-demand fetches.
- **Contextual direction status line:** the home screen shows "Auto-polling direction: To Work (morning window)" / "To Home (evening window)" / "To Work (last polled)" — updated on every `onResume()`. Context label disambiguates whether the direction is from the live active window or the last recorded poll.
...
│           │   ├── MainActivity.kt                 # SDK init, device discovery, CommuteProfileRepository load, manual direction toggle (Fetch Live only), auto-polling direction status line (contextual: "morning window" / "evening window" / "last polled"), Fetch Live pipeline, sendCommuteStatus(), debug "⋮" menu (hardcoded test payloads)
...
│           │   ├── PollingForegroundService.kt     # connectedDevice ForegroundService; AlarmManager exact-alarm scheduling; ACTION_WAKE_AND_POLL intent routing; PARTIAL_WAKE_LOCK for pipeline duration; smart scheduling (on-the-hour off-window + window-boundary truncation); poll Mutex; ConnectIQ + Gemini init; notification with last/next poll times; broadcasts ACTION_POLL_COMPLETED; resolvePollingDirection() derives direction from window index (0→TO_WORK, 1→TO_HOME), persists to KEY_LAST_POLLED_DIRECTION
...
│               ├── PollingSettingsTest.kt          # JSON round-trip (activeDays, backgroundPolling), backward-compat (old JSON → M–F + background ON defaults)
│               ├── PollingForegroundServiceSchedulingTest.kt  # getNextAlarmTime three-tier logic, nextOccurrenceOnActiveDay; resolvePollingDirection (morning/evening window, fallback, no windows, single window edge cases)
...
│       ├── DetailPageFactory.mc                    # ViewLoopFactory: builds page model from storage, measures hint height via fitTextToArea loop, chunks summary via DetailPagination, provides DetailPageView per page; header-only page 1 when hint fills screen
│       ├── DetailPageView.mc                       # Single-page view: structured header (title → badges → timestamp FONT_XTINY → hint in action color) + summary chunk in white; badges drawn with MtaColors.getLineColor() + dc.fillCircle()
...
- [x] FEAT-13: Garmin Detail View UX Revamp — Structured visual hierarchy on the detail view: title → badges → timestamp (FONT_XTINY) → reroute hint (action-tier color) → summary (white). Hint height is dynamically measured so the layout is always tight. When the hint fills the screen, page 1 is header-only and summary starts on page 2. For statuses without a hint, the hint section is omitted entirely.
- [x] FEAT-14: Add automatic commute direction handling so that the commute direction is passed to Gemini based on which polling window is currently active. Outside of an active polling window, use the last polled direction. Also improve the UX on the home screen of the android app so that the TO WORK and TO HOME buttons only control which direction is sent when you press FETCH LIVE. Improve the UX so that it is clear and intuitive how those settings work and that they relate to the FETCH LIVE button.
