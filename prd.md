...
- [ ] ARCH-01: Implement Hybrid Polling Architecture — Replace the coroutine `delay()` scheduling loop in `PollingForegroundService` with `AlarmManager` exact alarms. The `AlarmManager` will fire a `PendingIntent` directly to the active `connectedDevice` Foreground Service. This guarantees the hardware Real-Time Clock (RTC) wakes the CPU exactly on time to run the pipeline, resolving the issue of coroutines stalling during deep sleep. Retaining the active Foreground Service exempts the app from Android's 9-minute exact alarm Doze throttling and keeps the ConnectIQ SDK connection warm.
