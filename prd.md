...
- **`PollingForegroundService`** runs as a `connectedDevice` foreground service (Android 15 requires this type for BLE-connected apps; `dataSync` cannot start from `BOOT_COMPLETED` on Android 15+). Displays a persistent notification showing last poll time and next scheduled poll time, updated after every poll.
- **Coroutine-based scheduling loop:** `getNextDelayMs()` checks current time against configured commute windows — uses `intervalMinutes` during active windows, 60 minutes otherwise. Recalculates on every iteration so interval changes take effect immediately on the next poll.
...
│           │   ├── LinePickerBottomSheet.kt        # BottomSheetDialogFragment: 23 MTA lines as color-coded 44dp circular chips, grouped by trunk line; pre-selects current lines; Done callback
...
│           │   ├── PollingForegroundService.kt     # connectedDevice ForegroundService; coroutine polling loop with window-aware interval; ConnectIQ + Gemini init; notification with last/next poll times; broadcasts ACTION_POLL_COMPLETED
...
│           │   ├── PollingSettings.kt              # Data classes: CommuteWindow (isActive()), PollingSettings (enabled, windows, intervalMinutes); defaults: enabled=false, 8-9:30am + 5:30-7pm, 5min
│           │   ├── PollingSettingsRepository.kt    # Persists/loads PollingSettings to SharedPreferences as JSON
│           │   └── PollingSettingsActivity.kt      # On/off toggle + two TimePickerDialog window rows + interval Slider; Save starts/stops service immediately
│           ├── res/
...
│           │   ├── layout/activity_polling_settings.xml  # Polling settings: on/off toggle + window pickers + interval slider + Save button
...
│       ├── CommuteBuddyGlanceView.mc               # GlanceView (:glance): reads cs_action/cs_affected_routes, renders color-coded "Normal"/"Delays — N,W"/"Reroute — N,W"/"Stay Home"/"Waiting..."
...
│       ├── DetailPageView.mc                       # Single-page view: header (action, routes, hint, freshness) + summary chunk; waiting state when no cs_action
