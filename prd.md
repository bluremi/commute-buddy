...
- [ ] FEAT-10: Token usage optimization — Each Gemini request currently consumes ~2.5K input tokens (~700 system prompt + ~1,800 user prompt). Two inefficiencies identified: (1) The system prompt embeds both TO_WORK and TO_HOME commute profiles, but direction is hardcoded to TO_WORK — the TO_HOME legs (~80 tokens) are sent on every request but never evaluated. Fix: inject only the active direction's profile into the system prompt at model-init time or build-prompt time. (2) Real MTA feed descriptions for planned work (suspension notices, shuttle routes, ADA notices) can be 800-1,500 chars each; with 7 monitored routes there can be 5-8 active alerts simultaneously. The model makes decisions primarily from `header_text`; `description_text` adds nuance but exhibits rapidly diminishing returns past ~400 chars. Fix: cap `description_text` at ~400 chars in `buildPromptText()`. Combined these changes could reduce input tokens by 500-1,200 per request. Not urgent — at the current 50/day rate limiter cap the free tier is not at risk — but worth doing before FEAT-08 introduces automatic polling.
